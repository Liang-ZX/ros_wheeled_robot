...
class Mapping():
    ...
    def update(self, ox, oy, center_x, center_y):
        ...
        centix, centiy = int((center_x-minx) * 1.0 / xyreso), int((center_y-miny) * 1.0 / xyreso)
        ...
        for (x, y) in zip(ox, oy):
            ...
            ix, iy = int((x-minx)*1.0/xyreso), int((y-miny)*1.0/xyreso)
            ...
            for i in range(laser_beams.shape[0]):
                pos = laser_beams[i]
                tmp_p = pmap[int(pos[0]), int(pos[1])]
                if i == laser_beams.shape[0] - 1:
                    tmp_l =  np.log(tmp_p / (1-tmp_p)) + self.update_reflex
                else:
                    tmp_l =  np.log(tmp_p / (1-tmp_p)) + self.update_pass
                pmap[int(pos[0]), int(pos[1])] = 1 - 1.0 / (1+np.exp(tmp_l))
    ...
    def ray_casting(self,start, end):
        ...
        """
        points = np.array(start).reshape((1,2))
        x1, y1 = start
        x2, y2 = end
        ...
        while tx != x2:
            ...
            if P > 0:
                ...
                P = P + 2 * dy
            if not do_swap:
                points = np.vstack((points, np.array([tx, ty])))
            else:
                points = np.vstack((points, np.array([ty, tx])))
